...
\begin{itemize}
    ...
    按键映射如下：
    \begin{itemize}
        \item W/A/S/D：上/左/下/右移动
        \item J/Space/Enter：开火/开始游戏
        \item K：武器升级
        \item P：暂停/恢复
...
\begin{tikzpicture}[
    ...
    wideblock/.style = {
        ...
        minimum width = 13.0cm, minimum height = 0.9cm,
...
% ---- 第一层：输入 ----
\node[ioblock] (PS2) at (0, 0) {PS/2 键盘};

\node[block, fill=orange!10, right=1.5cm of PS2] (KBD) {Keyboard 模块\\(PS/2 Set 2 解码)};

\draw[arr] (PS2) -- (KBD);

% ---- 第二层：game_top ----
\node[wideblock, below=0.8cm of KBD, xshift=-1.5cm] (TOP)
    {game\_top（FPGA 顶层模块）};

\draw[arr] (KBD.south) -- (KBD |- TOP.north);

% ---- 第三层：核心子系统 ----
\node[coreblock, below=0.8cm of TOP, xshift=-3.2cm] (LOGIC)
    {\textbf{game\_logic}\\游戏逻辑引擎};
\node[vgablock, below=0.8cm of TOP, xshift=3.2cm] (VTOP)
    {\textbf{vga\_top}\\VGA 渲染管线};
\node[block, fill=orange!10, below=0.8cm of TOP, xshift=8.5cm] (CLKDIV)
    {clkdiv[31:0]\\时钟分频};

\draw[arr] (TOP.south) -- ++(0,-0.2) -| (LOGIC.north);
\draw[arr] (TOP.south) -- ++(0,-0.2) -| (VTOP.north);
\draw[arr] (TOP.south) -- ++(0,-0.2) -| (CLKDIV.north);

% ---- game_logic 子模块 ----
\node[subblock, fill=red!6, below left=0.6cm and -0.5cm of LOGIC] (FSM)  {5 状态 FSM};
\node[subblock, fill=red!6, below right=0.6cm and -0.5cm of LOGIC] (ENT)  {实体管理};
\node[subblock, fill=red!6, below=0.5cm of FSM] (COL)  {碰撞检测};
\node[subblock, fill=red!6, below=0.5cm of ENT] (LFSR) {LFSR 随机数};
\node[subblock, fill=red!6, below=0.5cm of COL, xshift=1.2cm] (SCR) {计分 \& 武器升级};

\draw[arr, shorten >=2pt] (LOGIC.south) -- ++(0,-0.1) -| (FSM.north);
\draw[arr, shorten >=2pt] (LOGIC.south) -- ++(0,-0.1) -| (ENT.north);
\draw[arr] (FSM) -- (COL);
\draw[arr] (ENT) -- (LFSR);
\draw[arr] (COL) -- (SCR);

% ---- vga_top 子模块 ----
\node[subblock, fill=green!6, below left=0.6cm and -0.5cm of VTOP] (VGAC)  {vgac 控制器};
\node[subblock, fill=green!6, below right=0.6cm and -0.5cm of VTOP] (REND)  {渲染器阵列};
\node[subblock, fill=green!6, below=0.5cm of VGAC] (MUX)   {优先级 MUX};
\node[subblock, fill=green!6, below=0.5cm of REND] (FTICK) {frame\_tick 生成};

\draw[arr, shorten >=2pt] (VTOP.south) -- ++(0,-0.1) -| (VGAC.north);
\draw[arr, shorten >=2pt] (VTOP.south) -- ++(0,-0.1) -| (REND.north);
\draw[arr] (VGAC) -- (MUX);
\draw[arr] (REND) -- (FTICK);
...
% ---- 第四层：输出设备 ----
\node[ioblock, below=1.2cm of SCR] (VGA)  {VGA 显示器\\(640×480@60Hz)};
\node[ioblock, below=1.2cm of COL, xshift=1.2cm] (SEG7) {7 段数码管};
\node[ioblock, below=1.2cm of LFSR, xshift=1.2cm] (LED)  {LED 指示灯};
\node[ioblock, below=1.2cm of FTICK] (P2S)  {串行 P2S 输出};

\draw[arr, dashed] (MUX.south) -- ++(0,-0.3) -| (VGA.north);
\draw[arr, dashed] (SCR.south) -- ++(0,-0.3) -| (SEG7.north);
\draw[arr, dashed] (SCR.south) -- ++(0,-0.3) -| (LED.north);
\draw[arr, dashed] (FTICK.south) -- ++(0,-0.3) -| (P2S.north);
...
\node[root] (TOP) {game\_top（FPGA 顶层）};

\node[branch, below left=1.0cm and -2.0cm of TOP] (CLK)  {clkdiv[31:0]（时钟分频）};
\node[branch, below right=1.0cm and -2.0cm of TOP] (KBD)  {Keyboard（PS/2 键盘驱动）};

\node[gleaf, below=1.0cm of CLK, xshift=-1.8cm] (LOGIC)
    {\textbf{game\_logic}（核心游戏逻辑）};
\node[vleaf, below=1.0cm of CLK, xshift=2.4cm] (VT)
    {\textbf{vga\_top}（VGA 渲染管线）};

\node[leaf, below=1.0cm of KBD, xshift=-1.8cm] (DISP)  {DispNum（直连 7 段）};
\node[leaf, below=1.0cm of KBD, xshift=2.4cm] (SEGDEV) {Seg7Device（串行 P2S）};

\node[subgleaf, below=0.8cm of LOGIC, xshift=-1.5cm] (FSM)   {5 状态 Moore FSM};
\node[subgleaf, below=0.8cm of LOGIC, xshift=1.5cm] (ENTS)   {实体管理};
\node[subgleaf, below=0.6cm of FSM] (COLL)   {逐对 AABB 碰撞检测};
\node[subgleaf, below=0.6cm of ENTS] (RNG)    {16-bit Galois LFSR};
\node[subgleaf, below=0.6cm of COLL, xshift=1.5cm] (SCORE) {计分 \& 武器升级};

\node[subvleaf, below=0.8cm of VT, xshift=-1.5cm] (VG)   {vgac（640×480@60Hz）};
\node[subvleaf, below=0.8cm of VT, xshift=1.5cm] (RENDERS) {渲染器阵列（generate 展开）};
\node[subvleaf, below=0.6cm of VG] (PRIO)    {优先级 MUX（if-else 链）};
\node[subvleaf, below=0.6cm of RENDERS] (FT) {frame\_tick（三级同步器）};
\node[subvleaf, font=\tiny, below=0.6cm of FT] (RPL) {player\_render ×1};
\node[subvleaf, font=\tiny, below=0.1cm of RPL] (REN)  {enemy\_render ×8};
\node[subvleaf, font=\tiny, below=0.1cm of REN]  (RPB)  {bullet\_render ×80};
\node[subvleaf, font=\tiny, below=0.1cm of RPB]  (ROB)  {obstacle\_render ×8};
\node[subvleaf, font=\tiny, below=0.1cm of ROB]  (RHU)  {hud\_render + menu\_render};

\node[subleaf, below=0.8cm of DISP] (SEGDEC)   {SegmentDecoder ×8};
\node[subleaf, below=0.8cm of SEGDEV] (SEGRMP) {Seg7Decode + Seg7Remap};
\node[subleaf, below=0.6cm of SEGRMP] (SHIFT)  {ShiftReg（并转串）};

\draw[arr] (TOP) -| (CLK);
\draw[arr] (TOP) -| (KBD);
\draw[arr] (CLK.south) -- ++(0,-0.3) -| (LOGIC.north);
\draw[arr] (CLK.south) -- ++(0,-0.3) -| (VT.north);
\draw[arr] (KBD.south) -- ++(0,-0.3) -| (DISP.north);
\draw[arr] (KBD.south) -- ++(0,-0.3) -| (SEGDEV.north);

\draw[arr] (LOGIC.south) -- ++(0,-0.2) -| (FSM.north);
\draw[arr] (LOGIC.south) -- ++(0,-0.2) -| (ENTS.north);
\draw[arr] (FSM) -- (COLL);
\draw[arr] (ENTS) -- (RNG);
\draw[arr] (COLL.south) -- ++(0,-0.15) -| (SCORE.north);

\draw[arr] (VT.south) -- ++(0,-0.2) -| (VG.north);
\draw[arr] (VT.south) -- ++(0,-0.2) -| (RENDERS.north);
\draw[arr] (VG) -- (PRIO);
\draw[arr] (RENDERS) -- (FT);
\draw[arr] (FT) -- (RPL);
\draw[arr] (RPL) -- (REN);
...
\draw[<->, >=Stealth, draw=red!70!black, thick]
    (LOGIC.east) -- node[midway, above, font=\footnotesize, text=red!70!black, rotate=90] {实体状态总线} (VT.west);
...
\node[draw=green!50!black, dashed, thick, inner sep=8pt,
      fit=(VT)(VG)(RENDERS)(PRIO)(FT)(RPL)(REN)(RPB)(ROB)(RHU),
...
\begin{tikzpicture}[
    ...
    cmt/.style = {
        font = \footnotesize, text = gray, align = left
    },
    arr/.style = {->, >=Stealth, thick},
...
% ---- 主流程 ----
\node[term] (START) {frame\_tick 脉冲};
\node[proc, below=0.7cm of START] (LFSR) {更新 LFSR};
\node[proc, below=0.7cm of LFSR] (INPUT) {检测 P/Tab 键边沿};
\node[proc, below=0.7cm of INPUT] (DEC) {cooldown/timer 递减};
\node[decision, below=0.7cm of DEC] (CHKSTATE) {state?};

% ---- MENU 分支（左） ----
\node[stproc, below left=0.8cm and -4.0cm of CHKSTATE] (MENU_INIT)
    {初始化实体寄存器};
\node[proc, below=0.6cm of MENU_INIT] (MENU_DIFF)
    {按键 1-4→难度\\M 键→模式};
\node[decision, below=0.6cm of MENU_DIFF] (MENU_START)
    {start\_key?};
\node[stproc, right=2.0cm of MENU_START] (MENU_WAIT)
    {等待开始};
...
\draw[larr] (CHKSTATE) -| node[pos=0.25, fill=white, font=\footnotesize] {MENU} (MENU_INIT);
\draw[arr] (MENU_INIT) -- (MENU_DIFF);
\draw[arr] (MENU_DIFF) -- (MENU_START);
\draw[arr] (MENU_START) -- node[above, font=\footnotesize] {否} (MENU_WAIT);
\draw[arr] (MENU_WAIT) |- ($(MENU_INIT.north -| MENU_WAIT.west)$) -- (MENU_INIT.north);

% ---- PLAYING 分支（右） ----
\node[stproc, below right=0.8cm and -4.5cm of CHKSTATE] (PLAY_MOVE)
    {玩家移动 (WASD)};
\node[proc, below=0.6cm of PLAY_MOVE] (PLAY_SHOOT)
    {玩家射击 (J/Space)};
\node[proc, below=0.6cm of PLAY_SHOOT] (PLAY_PBMOVE)
    {更新玩家子弹};
\node[proc, below=0.6cm of PLAY_PBMOVE] (PLAY_ENEMY)
    {敌机生成/移动/射击};
\node[proc, below=0.6cm of PLAY_ENEMY] (PLAY_EBMOVE)
    {更新敌方子弹};
\node[proc, below=0.6cm of PLAY_EBMOVE] (PLAY_OBSTACLE)
    {障碍物生成/下漂};
\node[proc, below=0.6cm of PLAY_OBSTACLE] (PLAY_COLLISION)
    {\textbf{碰撞检测} (AABB)};
\node[proc, below=0.6cm of PLAY_COLLISION] (PLAY_UPG)
    {计分/武器升级检查};

\node[decision, below=0.6cm of PLAY_UPG] (PLAY_CHKEND)
    {HP=0?\\或 score$\ge$500?};

\draw[larr] (CHKSTATE) -| node[pos=0.25, fill=white, font=\footnotesize] {PLAYING} (PLAY_MOVE);
...
\draw[arr] (PLAY_EBMOVE) -- (PLAY_OBSTACLE);
\draw[arr] (PLAY_OBSTACLE) -- (PLAY_COLLISION);
\draw[arr] (PLAY_COLLISION) -- (PLAY_UPG);
\draw[arr] (PLAY_UPG) -- (PLAY_CHKEND);

% ---- PAUSED 分支 ----
\node[stproc, right=3.0cm of CHKSTATE] (PAUSED_WAIT)
    {冻结游戏时间};
\node[decision, below=0.7cm of PAUSED_WAIT] (PAUSED_CHK)
    {P 键或开火键?};
\node[stproc, below=0.7cm of PAUSED_CHK] (PAUSED_RES)
    {恢复 PLAYING\\(250 帧冷却)};

\draw[larr] (CHKSTATE) -- node[above, font=\footnotesize] {PAUSED} (PAUSED_WAIT);
\draw[arr] (PAUSED_WAIT) -- (PAUSED_CHK);
\draw[arr] (PAUSED_CHK) -- node[right, font=\footnotesize] {是} (PAUSED_RES);

% ---- GAMEOVER 分支 ----
\node[stproc, below=1.2cm of DEC, xshift=-2.2cm] (GOVER_SHOW)
    {GAMEOVER\\显示 300 帧};
\node[decision, below=0.7cm of GOVER_SHOW] (GOVER_CHK)
    {timer=0?};
\node[stproc, below=0.7cm of GOVER_CHK] (GOVER_BACK)
    {清除实体\\返回 MENU};

\draw[arr] (CHKSTATE.south) -- ++(0,-0.5) -| node[pos=0.6, fill=white, font=\footnotesize, xshift=-0.7cm] {GAMEOVER} (GOVER_SHOW.north);
\draw[arr] (GOVER_SHOW) -- (GOVER_CHK);
\draw[arr] (GOVER_CHK) -- node[right, font=\footnotesize] {是} (GOVER_BACK);

% ---- WIN 分支 ----
\node[stproc, below=1.2cm of DEC, xshift=2.2cm] (WIN_SHOW)
    {YOU WIN\\显示 300 帧};
\node[decision, below=0.7cm of WIN_SHOW] (WIN_CHK)
    {timer=0?};
\node[stproc, below=0.7cm of WIN_CHK] (WIN_BACK)
    {清除实体\\返回 MENU};

\draw[arr] (CHKSTATE.south) -- ++(0,-0.5) -| node[pos=0.6, fill=white, font=\footnotesize, xshift=0.7cm] {WIN} (WIN_SHOW.north);
\draw[arr] (WIN_SHOW) -- (WIN_CHK);
\draw[arr] (WIN_CHK) -- node[right, font=\footnotesize] {是} (WIN_BACK);

% ---- 循环返回 ----
\node[term, below=1.5cm of WIN_BACK] (END) {更新 7 段/LED\\等待下一 tick};
\draw[arr, dashed] (MENU_WAIT.east) -- ++(1.0,0) |- (END.east);
\draw[arr, dashed] (PLAY_CHKEND.east) -- ++(0.8,0) |- node[font=\footnotesize, right] {否} (END.east);
\draw[arr, dashed] (PAUSED_CHK.east) -- ++(0.8,0) |- node[font=\footnotesize, right] {否} (END.east);
\draw[arr, dashed] (GOVER_CHK.east) -- ++(0.8,0) |- node[font=\footnotesize, right] {否} (END.east);
\draw[arr, dashed] (WIN_CHK.east) -- ++(0.8,0) |- node[font=\footnotesize, right] {否} (END.east);

\node[cmt, right=0.3cm of PLAY_CHKEND, text=red!70!black]
    {HP=0 → GAMEOVER};
...
\node[proc, right=0.8cm of CLK] (DIV) {cdiv[1:0] ÷4\\25MHz vga\_clk};
\node[vgac_box, right=0.8cm of DIV] (VGAC) {\textbf{vgac}\\VGA 控制器};

\node[proc, above=0.6cm of VGAC, fill=yellow!12] (SCAN) {row\_addr[8:0]\\col\_addr[9:0]};
\node[proc, below=0.6cm of VGAC, fill=yellow!12] (RDN) {rdn 消隐控制};
\node[io_box, right=2.5cm of VGAC] (VGAOUT) {RGB[3:0]×3\\HS / VS\\VGA 显示器};

% 渲染器阵列
\node[render_box, right=0.8cm of SCAN, yshift=1.2cm] (R_PL)  {player\_render};
\node[render_box, below=0.1cm of R_PL] (R_EN)  {enemy\_render ×8};
\node[render_box, below=0.1cm of R_EN]  (R_PB)  {bullet\_render ×80};
\node[render_box, below=0.1cm of R_PB]  (R_OB)  {obstacle\_render ×8};
\node[render_box, below=0.1cm of R_OB]  (R_HUD) {hud\_render};
\node[render_box, below=0.1cm of R_HUD] (R_MENU) {menu\_render};

\draw[arr_bus] (VGAC) -- (SCAN);
\draw[arr_bus] (VGAC) -- (RDN);
\draw[arr_bus] (SCAN.east) -- ++(0.3,0) |- (R_PL.west);
\draw[arr_bus] (SCAN.east) -- ++(0.3,0) |- (R_EN.west);
\draw[arr_bus] (SCAN.east) -- ++(0.3,0) |- (R_PB.west);
\draw[arr_bus] (SCAN.east) -- ++(0.3,0) |- (R_OB.west);
\draw[arr_bus] (SCAN.east) -- ++(0.3,0) |- (R_HUD.west);
\draw[arr_bus] (SCAN.east) -- ++(0.3,0) |- (R_MENU.west);

% 优先级 MUX
\node[mux_box, right=0.8cm of R_OB, yshift=-0.6cm] (MUX)
    {\textbf{优先级 MUX}\\Menu > HUD > Player\\> PBullet > EBullet\\> Enemy > Obstacle > BG};

\draw[arr] (R_PL.east)  -- ++(0.3,0) |- node[font=\tiny, pos=0.6, fill=white] {hit/col} (MUX.north);
\draw[arr] (R_EN.east)  -- (R_EN.east -| MUX.west);
\draw[arr] (R_PB.east)  -- (R_PB.east -| MUX.west);
\draw[arr] (R_OB.east)  -- (R_OB.east -| MUX.west);
\draw[arr] (R_HUD.east) -- (R_HUD.east -| MUX.west);
\draw[arr] (R_MENU.east) -- (R_MENU.east -| MUX.west);

\draw[arr_bus] (MUX.east) -- ++(0.4,0) |- node[pos=0.8, font=\tiny, fill=white] {vga\_data[11:0]} (VGAC.east);
\draw[arr] (VGAC) -- (VGAOUT);
\draw[arr] (CLK) -- (DIV);
\draw[arr] (DIV) -- (VGAC);

% ---- 下行：帧同步通路 ----
\node[sync_box, below=0.9cm of VGAC, yshift=-1.8cm] (VS_SYNC)
    {三级同步器\\vs\_sync[2:0]};
\node[sync_box, right=0.8cm of VS_SYNC] (FTICK)
    {frame\_tick 生成\\(vs 下降沿检测)\\约 60Hz 脉冲};
\node[hproc, right=0.8cm of FTICK] (LOGIC)
    {\textbf{game\_logic}\\游戏状态更新\\(100MHz 时钟域)};

\draw[arr_sync] (VGAC.south) -- ++(0,-0.5) -| node[pos=0.4, font=\tiny, fill=white] {vs} (VS_SYNC.north);
\draw[arr_sync] (VS_SYNC) -- (FTICK);
\draw[arr_sync] (FTICK) -- (LOGIC);

\draw[arr, bend left=35, draw=red!70!black, thick] (LOGIC.north) --
    node[midway, left, font=\footnotesize, text=red!70!black] {实体状态总线} (R_MENU.south);

% ---- 装饰注释 ----
\node[font=\small\bfseries, above=0.3cm of CLK, text=green!50!black]
    {VGA 像素渲染管线（25MHz vga\_clk 域）};
\node[font=\small\bfseries, below=0.3cm of VS_SYNC, text=red!60]
    {帧同步通路（100MHz 主时钟域）};
...
\subsection{游戏操作}
\begin{itemize}
    \item W/A/S/D：上/左/下/右移动玩家飞机
    \item J/Space/Enter：发射子弹（按住连发，受冷却限制）
    \item P：暂停/继续（单次触发，约 4 秒冷却）
    \item Tab：切换 7 段数码管显示内容（击毁数→生命值→积分→循环）
    \item K：武器升级（达到击杀阈值时按下）
\end{itemize}
...
\begin{lstlisting}
// Keyboard 模块输出：
// key_data[0]=W, key_data[1]=A, key_data[2]=S, key_data[3]=D
// key_data[4]=J, key_data[5]=Space, key_data[6]=Enter, key_data[7]=K
wire move_up    = key_data[0];   // W
wire move_left  = key_data[1];   // A
wire move_down  = key_data[2];   // S
wire move_right = key_data[3];   // D
wire fire_key   = key_data[4] || key_data[5] || key_data[6];  // J/Space/Enter
wire start_key  = fire_key;
wire upgrade    = key_data[7];   // K
\end{lstlisting}
...
`STATE_MENU: begin
    ...
    sel_mode <= key_mode ^ sel_mode;              // M 键切换
